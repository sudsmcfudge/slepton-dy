...
#    n_gen=`grep "All included subprocesses" $run/tag_1_pythia.log | tail -n 1 | gawk '{print $8}'`
    ...
    n_arr=(`./elec_tree.exe $run/tag_*_delphes.root "temp_elec.root" | tr "" "\n"`)
    n_pcuts=`./elec_cuts.exe "temp_elec.root" | tail -n 1`
...
#    n_pcuts=`./mt2_cuts.exe "temp.root" | tail -n 1`
    ...
    printf "%03d %03d %d %d %d %e\n" $mslep $mchi $n_pcuts $n_matched $n_gen $sigma_pb >> binoLSP_14TeV_table_full_elec.dat
