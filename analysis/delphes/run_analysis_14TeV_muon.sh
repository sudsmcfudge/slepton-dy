...
for run in $events_dir/run1_14TeV_mslep_*_mchi_*; do
#for run in $events_dir/run1_8TeV_mslep_1[3-4]5_mchi_100 $events_dir/run1_8TeV_mslep_140_mchi_100; do
    echo $run
...
#    n_gen=`grep "All included subprocesses" $run/tag_1_pythia.log | tail -n 1 | gawk '{print $8}'`
    ...
    n_arr=(`./muon_tree.exe $run/tag_*_delphes.root "temp_muon.root" | tr "" "\n"`)
    n_pcuts=`./muon_cuts.exe "temp_muon.root" | tail -n 1`
...
#    printf "%03d %03d " $mslep $mchi
#    echo $n_pcuts $n_matched $n_gen $sigma_pb >> binoLSP_8TeV_table_full_muon.dat
    printf "%03d %03d %d %d %d %e\n" $mslep $mchi $n_pcuts $n_matched $n_gen $sigma_pb >> binoLSP_14TeV_table_full_muon.dat
