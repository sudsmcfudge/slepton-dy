...
Double_t DR(Jet *jet, Electron *elec)
{
  
  Double_t deltaPhi, deltaEta, opAngle;  
  Double_t pi = TMath::Pi();
  
  deltaPhi = elec->Phi - jet->Phi;
  deltaEta = elec->Eta - jet->Eta;
  ...
  return TMath::Sqrt(opAngle*opAngle + deltaEta*deltaEta);
}
...
void makeTree(vector<TString> fn)
{
  ...
  myTree->Branch("max_fjetpt", &fjetpt, "fjetpt/D");
  ...
  for (Long64_t iev=0; iev<nEvents; iev++){
	...
	continue;
      ...
      // Find my jet candidates:
      for (i=0; i<njets; i++){

	jet1 = (Jet *) branchJets->At(i);

	if (TMath::Abs(jet1->Eta) < 4.5 and jet1->PT > 20){
	  ijet.push_back(i);
	}
      }
	...
	opAngle = TMath::Abs(deltaPhi);
      ...
      // if too close, disregard event
      if (deltaR <= drll){
	continue;
      }

      // check lepton/jet separation
      
      deltaljmin = 1e5;

      for (vector<int>::iterator ip=ijet.begin(); ip!=ijet.end(); ++ip){
	
	jet1 = (Jet *) branchJets->At(*ip);

	if ( DR(jet1, elec1) < deltaljmin ){
	  deltaljmin = DR(jet1, elec1);
	}

	if ( DR(jet1, elec2) < deltaljmin ){
	  deltaljmin = DR(jet1, elec2);
	}

	// already too close, move on
	if (deltaljmin <= drlj) break;
	...
	continue;
      ...
      // Find interior/forward signal jets and save max PTs
      for (vector<int>::iterator ip=ijet.begin(); ip!=ijet.end(); ++ip){
	...
	// forward jet
	else if (TMath::Abs(jet1->Eta) < 4.5) {
	  if (jet1->PT > fjetpt){
	    fjetpt = jet1->PT;
	  }
	}
	...
	dmetPhi = 2*pi - dmetPhi;
      ...
      // assume we only have 2 leading electrons (only 2 candidates)
      for (i=0; i<2; i++){
	elec_temp = (Electron *) branchElec->At(i);
	...
	if (delecPhi < dist){
      ...
      // find closest jet (of candidates)
      for (vector<int>::iterator ip=ijet.begin(); ip!=ijet.end(); ++ip){
